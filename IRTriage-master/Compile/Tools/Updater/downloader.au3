InetGet("https://github.com/AJMartel/IRTriageCMD/blob/master/cmd.exe?raw=true","Compile\Tools\cmd.exe")
;run("c:\temp\autoit-v3-setup.exe")
InetGet("http://www.7-zip.org/a/7z1514-extra.7z","c:\temp\7z1514-extra.7z")
InetGet("http://www.ltr-data.se/files/dosdev.zip","c:\temp\dosdev.zip")
InetGet("https://sourceforge.net/projects/md5deep/files/md5deep/md5deep-4.3/md5deep-4.3.zip/download","c:\temp\md5deep-4.3.zip")
InetGet("https://www.microsoft.com/en-us/download/confirmation.aspx?id=17657","c:\temp\rktools.exe")
InetGet("http://malware-hunters.net/wp-content/downloads/MFTDump_V.1.3.0.zip","c:\temp\MFTDump_V.1.3.0.zip")
InetGet("http://www.moonsols.com/downloads/1","c:\temp\moonsols_windows_memory_toolkit_community_edition.zip")
InetGet("http://www.nirsoft.net/utils/csvfileview.zip","c:\temp\csvfileview.zip")
InetGet("http://www.nirsoft.net/utils/winprefetchview.zip","c:\temp\winprefetchview.zip")
InetGet("https://github.com/keydet89/RegRipper2.8/archive/master.zip","c:\temp\RegRipper28.zip")
InetGet("https://download.sysinternals.com/files/sysinternalssuite.zip","c:\temp\sysinternalssuite.zip")
InetGet("http://unxutils.sourceforge.net/UnxUtils.zip","c:\temp\UnxUtils.zip")
InetGet("http://unxutils.sourceforge.net/UnxUpdates.zip","c:\temp\UnxUpdates.zip")


#comments-start
https://github.com/AJMartel/IRTriageCMD/blob/master/cmd.exe?raw=true
   Save to Compile\Tools\cmd.exe

http://www.7-zip.org/a/7z1514-extra.7z
   Extract 7za.exe to Compile\Tools\7za.exe
   Extract x64\7za.exe to Compile\Tools\7za64.exe

http://www.ltr-data.se/files/dosdev.zip
   Extract dosdev.exe to Compile\Tools\dosdev.exe

https://sourceforge.net/projects/md5deep/files/md5deep/md5deep-4.3/md5deep-4.3.zip/download
   Extract md5deep-4.3\md5deep.exe to Compile\Tools\md5deep.exe
   Extract md5deep-4.3\md5deep64.exe to Compile\Tools\md5deep64.exe
   Extract md5deep-4.3\sha1deep.exe to Compile\Tools\sha1deep.exe
   Extract md5deep-4.3\sha1deep64.exe to Compile\Tools\sha1deep64.exe

https://www.microsoft.com/en-us/download/confirmation.aspx?id=17657
   Extract robocopy.exe to Compile\Tools\robocopy.exe
   Extract x64\robocopy.exe to Compile\Tools\robo7.exe?

http://malware-hunters.net/category/tools/
http://malware-hunters.net/wp-content/downloads/MFTDump_V.1.3.0.zip

http://www.moonsols.com/downloads/1

http://www.nirsoft.net/utils/csvfileview.zip
http://www.nirsoft.net/utils/winprefetchview.zip

https://github.com/keydet89/RegRipper2.8/archive/master.zip

https://download.sysinternals.com/files/sysinternalssuite.zip

http://unxutils.sourceforge.net/UnxUtils.zip
http://unxutils.sourceforge.net/UnxUpdates.zip

#comment-end
